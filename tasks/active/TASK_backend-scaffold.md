...
## Acceptance criteria
- [ ] `GET /health` returns `{"status": "ok", "service": "api"}` with HTTP 200
- [ ] FastAPI app title is `api`
- [ ] App runs via `uvicorn` inside a Docker container
- [ ] `docker-compose.yml` brings up the backend service on port 8000
- [ ] `backend/requirements.txt` lists all dependencies with pinned versions
- [ ] No application logic beyond the health route — no stubs yet
