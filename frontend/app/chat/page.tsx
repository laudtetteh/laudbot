  ...
  return (
    <div className="flex flex-1 flex-col min-h-0 px-4 py-4 sm:px-6 sm:py-6">
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col min-h-0">
          ...
          <div className="flex flex-row items-center justify-between gap-3 sm:flex-col sm:items-end sm:gap-2">
            ...
            <div className="flex items-center gap-2 sm:self-end">
              ...
              <button
                ...
                className="rounded-md border border-zinc-800 px-3 py-1 text-xs text-zinc-500 transition-colors hover:border-zinc-600 hover:text-zinc-300"
              >
                Exit
              </button>
            </div>
        ...
        {switchConfirm && (
          <div className="animate-slide-down mb-3 rounded-xl border border-zinc-700/60 bg-zinc-900 px-4 py-4 flex-shrink-0">
        ...
        <div className={`messages-scroll mb-3 flex flex-1 min-h-0 flex-col gap-4 overflow-y-auto rounded-xl border border-zinc-800/60 bg-zinc-900/50 p-4 transition-opacity duration-200 ${switchConfirm ? "pointer-events-none opacity-30" : ""}`}>
          ...
          {!historyLoading && messages.length === 0 && !loading && (
            <div className="flex flex-1 flex-col items-center justify-center gap-6 py-8">
                ...
                <div className="flex flex-wrap justify-center gap-2 animate-fade-in-up">
                  {suggestedPrompts.map((prompt, i) => (
                    <button
                      ...
                      className="rounded-full border border-zinc-700/60 bg-zinc-800/60 px-4 py-2 text-xs text-zinc-400 transition-all hover:border-zinc-500 hover:bg-zinc-800 hover:text-zinc-200 active:scale-95"
                    >
                      {prompt}
                    </button>
                  ))}
              ...
              {suggestedPrompts.length === 0 && (
                <p className="text-xs text-zinc-700 animate-fade-in">Ask me anything.</p>
              )}
            </div>
          )}
          ...
          {loading && (
            <div className="animate-fade-in flex items-start">
              <div className="flex items-center gap-1 rounded-2xl rounded-bl-sm border border-zinc-700/40 bg-zinc-800/80 px-4 py-3">
          ...
          {/* Error */}
          {error && (
            <div className="animate-fade-in rounded-xl border border-red-900/60 bg-red-950/60 px-4 py-3 text-sm text-red-400">
              {error}
            </div>
          )}
        ...
        <div className={`flex gap-2 flex-shrink-0 sm:gap-3 transition-opacity duration-200 ${switchConfirm ? "pointer-events-none opacity-30" : ""}`}>
          <input
            ...
            className="flex-1 rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-3 text-sm text-white placeholder-zinc-600 outline-none transition-colors focus:border-zinc-600 disabled:text-zinc-500"
          />
          <button
            onClick={() => sendMessage()}
            disabled={loading || !input.trim() || !!switchConfirm}
            className="rounded-xl bg-zinc-700 px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-zinc-600 disabled:cursor-not-allowed disabled:bg-zinc-800 disabled:text-zinc-500 sm:px-5"
          >
            Send
          </button>
        </div>
