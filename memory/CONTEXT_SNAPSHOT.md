...
LaudBot is fully live and working end-to-end in production. Chat persistence, conversation history sidebar, and content env vars all shipped. System prompt and buddy overlay updated (stale `app.state`/recruiter references removed). All content is now in sync across local files, DB, and DO env vars.
...
### P1 — Features
1. Rate limiting (required before public deploy)

### P2 — UI/UX
2. Third theme option (dark/light/system or colour accent)

### P3 — Content / Knowledge
3. Expand knowledge base: deeper project writeups, personal trivia, LaudBot build story detail
4. System prompt tuning — repetitive answers in long chats

### P4 — Infrastructure
5. RAG / source ingestion pipeline (pgvector ready)
...
- Secrets: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `ADMIN_USERNAME`, `ADMIN_PASSWORD`, `JWT_SECRET_KEY`, `SYSTEM_PROMPT`, `OVERLAY_PROFESSIONAL`, `OVERLAY_PEER`, `OVERLAY_BUDDY`, `SUGGESTED_PROMPTS_PROFESSIONAL`, `SUGGESTED_PROMPTS_PEER`, `SUGGESTED_PROMPTS_BUDDY`, `FRONTEND_URL`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `DATABASE_URL`
...
- Tasks: `active/` has 1 file to move (TASK_conversation-id.md → done/); `done/` has 17 completed tasks
